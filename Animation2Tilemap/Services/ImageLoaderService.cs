...
public class ImageLoaderService(
    ...
    : IImageLoaderService
{
    private readonly string _inputPath = options.Input;
    ...
    private Dictionary<string, List<Image<Rgba32>>> LoadFromDirectory(string path, out bool suitableForAnimation)
    {
        ...
        stopwatch.Stop();
        logger.Verbose("Found {Count} file(s) in {Directory}. Took: {Elapsed}ms",
        ...
        logger.Information("Loaded {ImageCount} of {InputCount} file(s) containing a total of {FrameCount} frame(s). Took: {Elapsed}ms",
    ...
    private List<Image<Rgba32>> LoadFromFile(string file)
    {
        ...
        try
        {
            ...
            logger.Information("Loaded {FrameCount} frame(s) from {Path}", image.Frames.Count, file);
            return frames;
        }
        catch (UnknownImageFormatException)
        {
            logger.Warning("Unsupported format: {Path}", file);
            return frames;
        }
        catch (Exception ex)
        {
            logger.Error(ex, "Error loading image: {Path}", file);
            return frames;
        }
    }
    ...
    public bool TryLoadImages(out Dictionary<string, List<Image<Rgba32>>> images)
    {
        ...
        else
        {
            ...
            {
                var requestAnimation = confirmationDialogService.Confirm("The loaded images can be processed as animation frames. \n" +
                                                                          "Do you want to create an animation from these images?", true);
                if (requestAnimation)
                {
                    logger.Information("The loaded images will be processed as animation frames.");
                    TransformImagesToAnimation(ref images);
                }
                else
                {
                    logger.Information("The loaded images will be processed individually.");
                }
            }
            else
            {
                logger.Warning("The loaded images cannot be used as animation frames.");
            }
        }
        ...
        logger.Error("The input path does not lead to any valid images.");
        return false;
    }
    ...
    private void TransformImagesToAnimation(ref Dictionary<string, List<Image<Rgba32>>> images)
    {
        ...
        var name = namePatternService.GetMostNotablePattern(fileNames);
        if (name == null)
        {
        ...
        logger.Information("Using {Name} as the animation name.", name);
